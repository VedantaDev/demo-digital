...
const initialIssues: Issue[] = [
  { id: 'transportasi-aman', category: 'TRANSPORTASI', title: 'Transportasi umum yang aman untuk semua', description: 'Dorong pemerintah daerah mempercepat pembenahan penerangan, CCTV, dan kanal pengaduan di seluruh halte dan stasiun.', votes: 12748, target: 15000, supporters: 892, comments: 234, author: 'Ruang Aman Kota', time: '2 jam lalu', hot: true },
  { id: 'udara-jakarta', category: 'LINGKUNGAN', title: 'Hentikan polusi udara di Jakarta', description: 'Udara bersih bukan kemewahan. Kami menuntut transparansi sumber emisi dan tindakan tegas untuk kualitas udara kota.', votes: 9342, target: 12000, supporters: 614, comments: 167, author: 'Napas Jakarta', time: '5 jam lalu', hot: true },
  { id: 'pendidikan-gratis', category: 'PENDIDIKAN', title: 'Akses internet gratis untuk pelajar', description: 'Perjuangkan koneksi internet yang layak di sekolah negeri dan ruang belajar publik di seluruh Indonesia.', votes: 6718, target: 10000, supporters: 438, comments: 92, author: 'Kolektif Belajar', time: '1 hari lalu' },
  { id: 'ruang-hijau', category: 'KOTA', title: 'Kembalikan ruang hijau untuk warga', description: 'Tolak alih fungsi taman kota. Warga berhak atas ruang teduh, udara sehat, dan tempat berkumpul.', votes: 4287, target: 8000, supporters: 291, comments: 61, author: 'Taman Untuk Kita', time: '2 hari lalu' },
];
...
function Logo({ compact = false }: { compact?: boolean }) {
  return <Link href="/beranda" className={`flex items-center ${compact ? 'justify-center' : 'gap-3'} group`}><img src="/logo.png" alt="Demo Digital" className={compact ? 'h-9 w-9 object-cover object-left rounded' : 'h-10 w-[116px] object-contain object-left'} /><span className="sr-only">Demo Digital</span></Link>;
}
...
function Login() {
  ...
  return <div className="noise relative min-h-[100dvh] overflow-hidden bg-zinc-950">
    ...
    <img src="/hero.jpg" alt="" className="absolute inset-0 h-full w-full object-cover object-center opacity-55" />
...
function Beranda({ issues, setIssues }: { issues: Issue[]; setIssues: Dispatch<SetStateAction<Issue[]>> }) {
  ...
  return <Shell issuesCount={issues.length}><div className="mx-auto max-w-[1200px] px-5 py-8 md:px-10 md:py-12">
    <section className="animate-rise relative overflow-hidden border border-zinc-800 bg-zinc-900/50 p-6 md:p-10"><div className="absolute right-0 top-0 h-full w-1/2 opacity-30" style={{ backgroundImage: 'radial-gradient(circle at 80% 20%, #ff304f 0, transparent 1px)', backgroundSize: '18px 18px' }} /><div className="relative max-w-2xl"><div className="mb-5 flex items-center gap-2 text-[#ff6178]"><Zap size={15} fill="currentColor" /><span className="mono text-[10px] font-bold uppercase tracking-[.24em]">ruang isu nasional</span></div><h1 className="text-4xl font-semibold tracking-[-.05em] md:text-6xl">Keresahanmu<br /><span className="text-[#ff304f]">punya tempat.</span></h1><p className="mt-5 max-w-lg text-sm leading-6 text-zinc-400 md:text-base">Baca. Pilih. Bergerak. Setiap dukungan menambah tekanan yang terlihat dan membuat perubahan tak bisa diabaikan.</p><button onClick={() => setModal(true)} className="btn-primary mt-7 flex items-center gap-2 px-5 py-3 text-sm font-medium"><Plus size={17} /> Buat isu baru</button></div><div className="absolute bottom-5 right-7 hidden text-right md:block"><p className="mono text-[9px] uppercase tracking-[.2em] text-zinc-600">sinyal warga</p><p className="mt-1 text-4xl font-medium text-zinc-300">+24.8K</p></div></section>
...
export default function App() {
  return <div className="dark"><RouterView /></div>;
}